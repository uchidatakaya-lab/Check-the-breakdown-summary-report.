...
- `照合先集計方法` は本来 `最下科目式` を指定します。
- `照合先値指定` に `課税売上 10%+課税売上 (軽)8%` を入れてください。
- もし式を誤って `照合先集計方法` に入れてしまっても、最新版コードでは自動救済します。
- `照合先値指定` で明示的に `÷1000` したい場合は `(課税売上 10%+課税売上 (軽)8%)/1000` も指定可能です。
- 各項目に倍率をかける式も指定可能です。例: `(課税売上 10%/0.1)+(課税売上 (軽)8%/0.08)`
- `_千円` 項目による自動 `÷1000` と、`照合先値指定` 側の `/1000` 指定はいずれも切り捨てです（重複適用はしません）。
- 文字列照合／固定値照合は `一致方法`（完全一致・部分一致・前方一致・後方一致）を使用します。住所は `部分一致` 設定が可能です。
